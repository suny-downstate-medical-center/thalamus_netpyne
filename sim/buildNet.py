...
# --- Connect using cylinder with exponential decay or uniform distribution
...
for conn_type in connect_pops.keys():
    for (pre_pop,post_pop,secList) in connect_pops[conn_type]:
        if   conn_type=='cylinder_exp':(conn_method,conn_rule)=BN.BuildNetwork.cylinderProjection_expDecay(pre_pop,post_pop)
        elif conn_type=='cylinder_uni':(conn_method,conn_rule)=BN.BuildNetwork.cylinderProjection_uniform( pre_pop,post_pop)
