...
if cfg.connType == 'original':
    ...
    # --- Changing iT shift for all sections in the biophysical cell models
    if cfg.it_shift is not None:
        print('\t>>\t --- Test:   Changing iT shift (Iavarone, 2019) ', str(cfg.it_shift))
        for biophys_cell in netParams.cellParams.keys():
            for sec in netParams.cellParams[biophys_cell]['secs'].keys():
                for mech in netParams.cellParams[biophys_cell]['secs'][sec]['mechs'].keys():
                    if mech == 'TC_iT_Des98':
                        # previous_shift = netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['shift']
                        netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['shift']+= cfg.it_shift # mV
                        # new_shift = netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['shift']
                        # print('Changing shift from ', str(previous_shift), ' to ', str(new_shift), ' \t sec: ', sec)

    # --- Changing iT actshift for all sections in the biophysical cell models
    if cfg.it_actshift is not None:
        print('\t>>\t --- Test:   Changing iT actshift (Iavarone, 2019) ', str(cfg.it_actshift))
        for biophys_cell in netParams.cellParams.keys():
            for sec in netParams.cellParams[biophys_cell]['secs'].keys():
                for mech in netParams.cellParams[biophys_cell]['secs'][sec]['mechs'].keys():
                    if mech == 'TC_iT_Des98':
                        # previous_shift = netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['shift']
                        netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['actshift']+= cfg.it_actshift # mV
                        # new_shift = netParams.cellParams[biophys_cell]['secs'][sec]['mechs'][mech]['shift']
                        # print('Changing shift from ', str(previous_shift), ' to ', str(new_shift), ' \t sec: ', sec)
