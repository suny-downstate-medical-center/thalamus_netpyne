...
class Conversion():
    ...
    def getAfferentSectionIds(morphologyFullPath,single_soma_sec=True):
        ...
        IDs must start at 1                                         <----------------- VERY IMPORTANT
        ...
        For the purpose of mapping information to morphologies, each section has a designated section ID, which is a integer in [0, #sections - 1]. 
        The soma is always section 0. The rest of the sections are first grouped by section type in this order: 
            1 = axon, 2 = basal and 3 = apical. 
        All sections in a group are given IDs incrementally, starting at the last ID from the previous section plus 1. 
        The order in which sections are assigned an ID is the order in which their first segment appears in the file. 
        '''
        ...
        # --- Flag to fix a bug where first pt3d is created as soma_0 and other points are created as soma_1
        if single_soma_sec:
            raw_all_secs_fix = [sec for sec in raw_all_secs if (sec == 'soma_0') or ('soma' not in sec)]
            del raw_all_secs
            raw_all_secs = raw_all_secs_fix
        ...
        for key in rsec_name_dict.keys():
            final_id_ = sum(key in rsec_name for rsec_name in raw_all_secs)+final_id_; 
            final_ids.append((key,final_id_))
        ...
        for key,final_id in final_ids:
            for sec_ind,id in enumerate(range(start_id,final_id)):
                # print((key+'_'+str(sec_ind),id))
                full_ids_list.append((key+'_'+str(sec_ind),id))
            start_id = final_id
        # # --- Shifts ids by 1, because the SONATA reference starts at 1 instead of 0
        # for sec_id_index, (sec,id) in enumerate(full_ids_list):
        #     full_ids_list[sec_id_index]=(sec,id+1)
...
class Utils():
        ...
        for (parSec,sec) in map_secs.keys():
            if parSec == root_sec:soma_pathDist.update({sec:map_secs[(parSec,sec)]})
                ...
                try:    soma_pathDist.update({sec:map_secs[(parSec,sec)]+soma_pathDist[parSec]})
                    ...
                    failed.append(sec)
                    failed_pair.append((parSec,sec))
        
        # --- Retry calculating distance, in case parent section was not in the list in the first iteration
        failed_again=[]
        for (parSec,sec) in failed_pair:
            if parSec == root_sec:soma_pathDist.update({sec:map_secs[(parSec,sec)]})
            else:
                try:    soma_pathDist.update({sec:map_secs[(parSec,sec)]+soma_pathDist[parSec]})
                except: failed_again.append(sec)
        if len(failed_again)>0:print('Failed again sections: ',failed)
    ...
    # def secListFromPathDistance(soma_pathDist,distThresh=[10,75],basalDendrites=[],repeats=None):
    def secListFromPathDistance(soma_pathDist,distThresh={'proximal':[0,10],'intermediate':[5,75],'distal':[20,1000],},basalDendrites=[],repeats=None):
        secLists_dict={
                        'inputs__proximal':[],
        ...
        #     else: continue
        for sec in soma_pathDist.keys():
            print('\n\n',sec, soma_pathDist[sec])
            if 'soma' in sec:                       
                print('somatic')
                secLists_dict['inputs__proximal'].append(sec)
            if sec in basalDendrites:             
                print('basal')
                secLists_dict['inputs__proximal'].append(sec)
            
            if (soma_pathDist[sec]< distThresh['proximal'][1]):         
                print('proximal')
                secLists_dict['inputs__proximal'].append(sec)
            if  soma_pathDist[sec]>=distThresh['intermediate'][0] \
              and soma_pathDist[sec]< distThresh['intermediate'][1]:    
                print('intermediate')
                secLists_dict['inputs__intermediate'].append(sec)
            if (soma_pathDist[sec]>=distThresh['distal'][0]):           
                print('distal')
                secLists_dict['inputs__distal'].append(sec)
            

        if repeats is not None:
            print('\t--- Expanding secLists by ', str(repeats), ' repeats')
            secLists_dict_={}
            for key, values in secLists_dict.items():
                expanded_values = [item for item in values for _ in range(repeats)]
                secLists_dict_[key] = expanded_values
            del secLists_dict
            secLists_dict=secLists_dict_
            ...
            for prePop in popParams.keys():
                if prePop.split('__')[0] in source_pop:
                    try:
                        ...
                        continue
                        # store_syn_drive[source_pop][spkt]+=0 # --- (quick patch 2023-12-02 - worked fine) - skip adding if no spikes were present in the population
        if plotFig:
            legend_list=[]
            color_list=[]
            pathway_colors_dict = { 
                                    ...
                                    'VPL_TC':                       'k',
                                    'MedialLemniscus_projections':  'olive',
    ...
    def linear_extrapolation(x, y, x_new):
        # Find the slope of the line using the first and last known points
        slope = (y[-1] - y[0]) / (x[-1] - x[0])
        
        # Estimate y values for x_new using linear extrapolation
        y_new = y[-1] + slope * (x_new - x[-1])
        return y_new

    # Fit the data to a quadratic equation
    def quadratic_extrapolation(x, y, x_new):
        x_new_array = np.array(x_new)
        coefficients_quad = np.polyfit(x, y, 2)
        quadratic_eq = np.poly1d(coefficients_quad)
        y_new = quadratic_eq(x_new_array)
        return y_new

    # Fit the data to a cubic equation
    def cubic_extrapolation(x, y, x_new):
        x_new_array = np.array(x_new)
        coefficients_cubic = np.polyfit(x, y, 3)
        cubic_eq = np.poly1d(coefficients_cubic)
        y_new = cubic_eq(x_new_array)
        return y_new
...
class ConvertSynapses():
    def getTargetSections(cell,node_pathway_edges,full_ids_list):
        ...
        for edge_name in node_pathway_edges.keys():
            ...
            # print(afferent_section_ids)
            secList = [SONATA_dict['SONATA_id_sec'][aff_sec_id] for aff_sec_id in afferent_section_ids]
            # secList = [SONATA_dict['SONATA_id_sec'][aff_sec_id] for aff_sec_id in afferent_section_ids if aff_sec_id in SONATA_dict['SONATA_id_sec'].keys()]
            # print(secList)
            ...
            for column in list(df_sec.columns):
                if column not in node_pathway_edges[edge_name].columns:
                    ...
                    # print(value)
                    # for sec_ind,sec in enumerate(value): 
                    #     if sec not in cell['secs'].keys(): value[sec_ind]='soma_0'
                    # print(value)
    ...
    def checkSyns(cell,cell_properties,node_pathway_edges):
        ...
        return node_pathway_edges
    def plotCellShape(cell,extra_flag='_cellShape',cell_label=None,savefolder=None,axis_off=True, morph_color='k'):
        import matplotlib.pyplot as plt
        ...
        for sec in cell['secs'].keys():
            ...
            if len(cell['secs'][sec]['geom']['pt3d'])>0 and type(cell['secs'][sec]['geom']['pt3d'])==list:
                for ind_pt3d, pt3d in enumerate(cell['secs'][sec]['geom']['pt3d']):
                    ...
                    ax.plot([previous_pt3d[0],pt3d[0]],[previous_pt3d[1],pt3d[1]],[previous_pt3d[2],pt3d[2]],linewidth=pt3d[3],color=morph_color, alpha=0.7)
        ...
        if savefolder is not None:
            plt.savefig(savefolder+'/'+'Conversion_cellSynapses_'+cell_label+extra_flag+'.png',dpi=1000)
        else:
    ...
    def plotSynapseLocation(cell,cell_properties,node_pathway_edges,full_ids_list, extra_flag='',showEstimadedSyns=True,plotSimplified=True):
        ...
        for edge_name in node_pathway_edges.keys():
            if   'MedialLemniscus_projections'  in edge_name:   c_ = 'olive'
            # if   'MedialLemniscus_projections'  in edge_name:   c_ = 'k'
            elif 'thalamus_neurons|VPL_TC'      in edge_name:   c_ = 'g'
            elif 'thalamus_neurons|VPL_IN'      in edge_name:   c_ = 'hotpink'
            elif 'thalamus_neurons|Rt_RC'       in edge_name:   c_ = 'blue'
            elif 'CorticoThalamic_projections'  in edge_name:   c_ = 'r'
            else:                                               c_ = 'orange'
...
#     def apply_inverse_rotation(point, cell_properties, apply_rotation_x, apply_rotation_y, apply_rotation_z):
#         # # Combine the rotation matrices
#         # R_x = RotateMorphology.rotation_matrix_x(cell_properties.rotation_angle_xaxis)
#         # R_y = RotateMorphology.rotation_matrix_y(cell_properties.rotation_angle_yaxis)
#         # R_z = RotateMorphology.rotation_matrix_z(cell_properties.rotation_angle_zaxis)

#         # # Combined rotation matrix
#         # R = R_z @ R_y @ R_x

#         # Combine the rotation matrices based on the flags
#         R = np.eye(3)
#         if apply_rotation_x:
#             R = RotateMorphology.rotation_matrix_x(-cell_properties.rotation_angle_xaxis) @ R
#         if apply_rotation_y:
#             R = RotateMorphology.rotation_matrix_y(-cell_properties.rotation_angle_yaxis) @ R
#         if apply_rotation_z:
#             R = RotateMorphology.rotation_matrix_z(-cell_properties.rotation_angle_zaxis) @ R

#         # Inverse of the combined rotation matrix
#         R_inv = np.linalg.inv(R)

#         xyz = np.array(point[:3])  # Extract x, y, z
#         xyz_rotated = R_inv @ xyz  # Apply the inverse rotation
#         return xyz_rotated.tolist() + [point[3]]  # Append diameter (d) unchanged

#     # Apply the inverse rotation to each section's coordinates
#     # def rotate(cell,cell_properties):
#     def rotate(cell, cell_properties, apply_rotation_x=False, apply_rotation_y=False, apply_rotation_z=True):
#         # Initialize an empty dictionary to store the rotated sections
#         rotated_secs = {}
#         # Apply the inverse rotation to each section's coordinates
#         for sec in cell['secs']:
#             rotated_secs.update({sec: []})
#             pt3d_list = cell['secs'][sec]['geom']['pt3d']
#             for point in pt3d_list:
#                 rotated_point = RotateMorphology.apply_inverse_rotation(point, cell_properties, apply_rotation_x=False, apply_rotation_y=True, apply_rotation_z=True)
#                 rotated_secs[sec].append(rotated_point)
#         return rotated_secs
...
#     def apply_inverse_rotation(point, cell_properties):
#         # Combine the rotation matrices
#         R_x = RotateMorphology.rotation_matrix_x(cell_properties.rotation_angle_xaxis)
#         R_y = RotateMorphology.rotation_matrix_y(cell_properties.rotation_angle_yaxis)
#         R_z = RotateMorphology.rotation_matrix_z(cell_properties.rotation_angle_zaxis)
...
#         # Adjust for cell orientation
#         xyz_rotated[0] -= cell_properties.orientation_x
#         xyz_rotated[1] -= cell_properties.orientation_y
#         xyz_rotated[2] -= cell_properties.orientation_z
...
#             pt3d_list = cell['secs'][sec]['geom']['pt3d']
#             for point in pt3d_list:
#                 rotated_point = RotateMorphology.apply_inverse_rotation(point, cell_properties)
#                 rotated_secs[sec].append(rotated_point)
#         return rotated_secs
...
class RotateMorphology():
    ...
    def apply_rotation_z(point, angle):
        # Apply the rotation around the z-axis
        R_z = RotateMorphology.rotation_matrix_z(angle)
        xyz = np.array(point[:3])  # Extract x, y, z
        xyz_rotated = R_z @ xyz  # Apply the rotation
        return xyz_rotated.tolist() + [point[3]]  # Append diameter (d) unchanged

    # Apply the rotation around the z-axis to each section's coordinates
    def rotate(cell, angle=1.5707963267948966):
        # Initialize an empty dictionary to store the rotated sections
        rotated_secs = {}
        # Apply the rotation around the z-axis to each section's coordinates
        for sec in cell['secs']:
            ...
            for point in pt3d_list:
                # Apply the rotation around the z-axis
                rotated_point = RotateMorphology.apply_rotation_z(point, angle)
                rotated_secs[sec].append(rotated_point)
        return rotated_secs
...
class LoadBBPCircuit():
    ...
    def cell_properties_to_dict(cell_properties):
        return {cell_properties['mtype']+'__'+str(cell_properties.name):cell_properties.to_dict()}
    ...
    def modTemplate(edge_data,mod_file):
        ...
        if mod_file =='ProbAMPANMDA_EMS_original':
            edge_dict={ 'mod':          'ProbAMPANMDA_EMS_original',
                        ...
                        'gmax':         edge_data['conductance'],    # test: 2023_11_23     # IMPORTANT: .001 IS A CONVERSION FACTOR FROM nS to uS AND MUST BE ADDED
                        # 'gmax':         edge_data['conductance']*.001,    # test: 2023_11_23     # IMPORTANT: .001 IS A CONVERSION FACTOR FROM nS to uS AND MUST BE ADDED
        ...
        # --- Use this gap junction instead
            ...
            edge_dict=  {   'mod': 'ElectSyn',
                            'g':            edge_data['conductance']*0.001, # conversion from nanosiemens (paper) to microsiemens (mod file units)
                            'pointerParams': {
                                'target_var':  'vpeer',
    ...
    def modifySyn(circuit_dict,edge_name):
        ...
        if edge_mech == 'chemical': 
            if ('CorticoThalamic' in edge_source_pop):
                if edge_target_pop   == 'VPL_TC':           modif_dict = {'mg':1, 'NMDA_ratio': 1.91}
                elif edge_target_pop == 'Rt_RC':            modif_dict = {'mg':1, 'NMDA_ratio': 0.99}
                else:                                       modif_dict = {'mg':1, 'NMDA_ratio': 0.57} # mosaic modification
            elif ('MedialLemniscus' in edge_source_pop):
                if edge_target_pop   == 'VPL_TC':           modif_dict = {'mg':1, 'NMDA_ratio': 0.41}
            else:
                if 'thalamus_neurons' in edge_source_pop:
                    # --- Identifies if the synapse SOURCE is VPL_TC
                    if   edge_source_pop.split('|')[1]=='VPL_TC':   
                        if edge_target_pop == 'Rt_RC':              modif_dict = {'mg':1, 'NMDA_ratio': 0.57}
                        ...
                        else:                                       modif_dict = {'e_GABAA':-82,'e_GABAB':-97, 'GABAB_ratio': 0, 'tau_d_GABAB':77} # mosaic modification
...
class ConvertMorphology():
    ...
    def mergeSomaSections(cell,root_soma_sec = 'soma_0'):
        # --- Identifies the soma sections
        soma_sections=[sec_name for sec_name in cell['secs'].keys() if 'soma' in sec_name]
        
        # --- Checks wether the soma_0 section is defined as a point
        if len(soma_sections)>1 and len(cell['secs'][root_soma_sec]['geom']['pt3d'])==3: 
            print('\t--- Cell was likely created with a circular soma\n\t--- Applying morphology fix')
        else:
            print('\t--- WARNING: Cell doesnt meet the criteria for soma remapping (being a point) - Check morphology')
            return cell

        # --- Creates a list with the central point for soma_0 and the other points for the soma sections to make a single pt3d list
        root_point = [cell['secs'][root_soma_sec]['geom']['pt3d'][1]]
        other_points = []
        for sec_name in soma_sections:
            if sec_name == root_soma_sec:continue
            other_points.extend(cell['secs'][sec_name]['geom']['pt3d'])
        pt3d_points = root_point+other_points

        # --- Calculate segment length and total segment length
        segment_length = 0
        prev_point = None
        ...
        # --- Calculate weighted average diameter
        weighted_sum = 0
        prev_point = None
        for point in pt3d_points:
            if prev_point is not None:
                ...
                # Add weighted diameter
                weighted_sum += dist * point[3]
            prev_point = point
        weighted_average_diameter = weighted_sum / segment_length

        # --- Creates a new soma section with updated pt3d, length and weighted diameter (considering the different distances between 3d points)
        new_soma = Utils.my_dict_copy(cell['secs'][root_soma_sec])
        new_soma['geom']['L']       = segment_length
        new_soma['geom']['diam']    = weighted_average_diameter
        new_soma['geom']['pt3d']    = pt3d_points
        # --- Creates a new dictionary of sections without soma
        new_secs = {key: val for key, val in cell['secs'].items() if key not in soma_sections}
        # --- Updates the dictionary of sections with the new soma
        new_secs.update({root_soma_sec:new_soma})
        # --- Updates the cell dictionary with the new sections
        cell['secs']=new_secs

        return cell


        # root_soma = Utils.my_dict_copy(cell['secs'][root_soma_sec])

        # for sec_name in soma_sections:
        #     if sec_name=='soma_0':continue
        #     root_soma['geom']['pt3d'].extend(cell['secs']['soma_1']['geom']['pt3d'])
        #     merged_soma['geom']['L'] += soma_1['geom']['L']
...
class StoreParameters():
    ...
    def getS1L6SynMechs(suppress_output=True):
        if not suppress_output:print('Loading data for S1 synaptic mechanisms')
        TableS1SynMechs={   
                            'S1_II_STP_Det_0':          {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.16,  'Dep': 45.0,   'Fac': 376.0, 'tau_d_GABAA': 10.4, 'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_II_STP_Det_1':          {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.25,  'Dep': 706.0,  'Fac': 21.0,  'tau_d_GABAA': 8.3,  'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_IE_STP_Det_5':          {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.32,  'Dep': 144.0,  'Fac': 62.0,  'tau_d_GABAA': 6.44, 'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_IE_STP_Det_9':          {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.3,   'Dep': 1250.0, 'Fac': 2.0,   'tau_d_GABAA': 8.3,  'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_IE_STP_Det_10':         {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.14,  'Dep': 875.0,  'Fac': 22.0,  'tau_d_GABAA': 8.3,  'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_IE_STP_Det_13':         {'mod': 'ProbGABAAB_EMS_original',   'Use': 0.415, 'Dep': 162.0,  'Fac': 690.0, 'tau_d_GABAA': 10.4, 'tau_r_GABAA': 0.2, 'tau_d_GABAB': 260.9, 'tau_r_GABAB': 3.5},
                            'S1_EI_STP_Det_115':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.02,  'Dep': 194.0,  'Fac': 507.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.4, 'mg': 1.0},
                            'S1_EE_STP_Det_116':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.5,   'Dep': 671.0,  'Fac': 17.0,  'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.8, 'mg': 1.0},
                            'S1_EE_STP_Det_126':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.21,  'Dep': 460.0,  'Fac': 230.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.8, 'mg': 1.0},
                            'S1_EE_STP_Det_127':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.391, 'Dep': 780.0,  'Fac': 51.0,  'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.8, 'mg': 1.0},
                            'S1_EE_STP_Det_128':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.27,  'Dep': 559.0,  'Fac': 200.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.8, 'mg': 1.0},
                            'S1_EE_STP_Det_129':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.222, 'Dep': 535.0,  'Fac': 116.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.8, 'mg': 1.0},
                            'S1_EI_STP_Det_131':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.58,  'Dep': 240.0,  'Fac': 71.0,  'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.4, 'mg': 1.0},
                            'S1_EI_STP_Det_132':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.01,  'Dep': 242.0,  'Fac': 563.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.4, 'mg': 1.0},
                            'S1_EI_STP_Det_133':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.09,  'Dep': 138.0,  'Fac': 670.0, 'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.4, 'mg': 1.0},
                            'S1_EI_STP_Det_134':        {'mod': 'ProbAMPANMDA_EMS_original', 'Use': 0.72,  'Dep': 227.0,  'Fac': 14.0,  'tau_d_AMPA': 1.74,  'tau_r_AMPA': 0.2,  'tau_r_NMDA': 0.29,   'tau_d_NMDA': 43, 'NMDA_ratio': 0.4, 'mg': 1.0},
                            }
        return TableS1SynMechs
    
    def getS1L6Connectivity(suppress_output=True,L6_CT_byophisical_pops=['L6A_activated', 'L6A_suppressed', 'L6A_sparse', 'L6A_silent'],pop_flag='__pop'):
        if not suppress_output:print('Loading data for S1 connectivity rules')
        TableS1Conns = {    'II_L6_LBC_L6_LBC':         {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'synMech': 'S1_II_STP_Det_0',   'probability': '0.05653*exp(-dist_2D/68.68)*(dist_2D<375)', 'weight': 0.83, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'II_L6_LBC_L6_MC':          {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_II_STP_Det_1',   'probability': '0.06624*exp(-dist_2D/73.61)*(dist_2D<375)', 'weight': 0.83, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_TPC_L1':      {'preConds': {'pop': ['L6IN_LBC_cNA']},                                     'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_IE_STP_Det_5',   'probability': '0.06747*exp(-dist_2D/64.57)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 13, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_TPC_L1_B':    {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA']},                     'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_IE_STP_Det_10',  'probability': '0.06747*exp(-dist_2D/64.57)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 13, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_TPC_L4':      {'preConds': {'pop': ['L6IN_LBC_cNA']},                                     'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_IE_STP_Det_5',   'probability': '0.07076*exp(-dist_2D/65.70)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_TPC_L4_B':    {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA']},                     'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_IE_STP_Det_10',  'probability': '0.07076*exp(-dist_2D/65.70)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_BPC':         {'preConds': {'pop': ['L6IN_LBC_cNA']},                                     'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_5',   'probability': '0.07553*exp(-dist_2D/68.52)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_BPC_B':       {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA']},                     'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_10',  'probability': '0.07553*exp(-dist_2D/68.52)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_IPC':         {'preConds': {'pop': ['L6IN_LBC_cNA']},                                     'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_5',   'probability': '0.05797*exp(-dist_2D/73.43)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_IPC_B':       {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA']},                     'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_10',  'probability': '0.05797*exp(-dist_2D/73.43)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_UTPC':        {'preConds': {'pop': ['L6IN_LBC_cNA']},                                     'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_IE_STP_Det_5',   'probability': '0.07833*exp(-dist_2D/68.55)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 13, 'sec': 'spiny'},
                            'IE_L6_LBC_L6_UTPC_B':      {'preConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA']},                     'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_IE_STP_Det_10',  'probability': '0.07833*exp(-dist_2D/68.55)*(dist_2D<375)', 'weight': 0.91, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 13, 'sec': 'spiny'},
                            'II_L6_MC_L6_LBC':          {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'synMech': 'S1_II_STP_Det_1',   'probability': '0.06193*exp(-dist_2D/52.98)*(dist_2D<375)', 'weight': 0.83, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 10, 'sec': 'spiny'},
                            'II_L6_MC_L6_MC':           {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_II_STP_Det_1',   'probability': '0.07371*exp(-dist_2D/54.63)*(dist_2D<375)', 'weight': 0.83, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 10, 'sec': 'spiny'},
                            'IE_L6_MC_L6_TPC_L1':       {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_IE_STP_Det_9',   'probability': '0.08512*exp(-dist_2D/50.21)*(dist_2D<375)', 'weight': 0.75, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_MC_L6_TPC_L4':       {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_IE_STP_Det_9',   'probability': '0.09312*exp(-dist_2D/49.37)*(dist_2D<375)', 'weight': 0.75, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'IE_L6_MC_L6_BPC':          {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_9',   'probability': '0.08791*exp(-dist_2D/53.07)*(dist_2D<375)', 'weight': 0.75, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 11, 'sec': 'spiny'},
                            'IE_L6_MC_L6_IPC':          {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_IE_STP_Det_13',  'probability': '0.06609*exp(-dist_2D/54.80)*(dist_2D<375)', 'weight': 0.75, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 10, 'sec': 'spiny'},
                            'IE_L6_MC_L6_UTPC':         {'preConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_IE_STP_Det_9',   'probability': '0.08842*exp(-dist_2D/55.15)*(dist_2D<375)', 'weight': 0.75, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 12, 'sec': 'spiny'},
                            'EI_L6_TPC_L1_L6_LBC':      {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'synMech': 'S1_EI_STP_Det_132', 'probability': '0.06187*exp(-dist_2D/49.21)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 9, 'sec': 'spiny'},
                            'EI_L6_TPC_L1_L6_MC':       {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_EI_STP_Det_133', 'probability': '0.07273*exp(-dist_2D/53.24)*(dist_2D<375)', 'weight': 0.11, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EE_L6_TPC_L1_L6_TPC_L1':   {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_EE_STP_Det_126', 'probability': '0.08642*exp(-dist_2D/60.15)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 5, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L1_L6_TPC_L4':   {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_EE_STP_Det_126', 'probability': '0.07845*exp(-dist_2D/64.04)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 5, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L1_L6_BPC':      {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.11677*exp(-dist_2D/70.02)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L1_L6_IPC':      {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.09046*exp(-dist_2D/74.98)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L1_L6_UTPC':     {'preConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_EE_STP_Det_116', 'probability': '0.12337*exp(-dist_2D/69.39)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EI_L6_TPC_L4_L6_LBC':      {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'synMech': 'S1_EI_STP_Det_132', 'probability': '0.06559*exp(-dist_2D/47.00)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 9, 'sec': 'spiny'},
                            'EI_L6_TPC_L4_L6_MC':       {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_EI_STP_Det_133', 'probability': '0.07643*exp(-dist_2D/53.52)*(dist_2D<375)', 'weight': 0.11, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EE_L6_TPC_L4_L6_TPC_L1':   {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_EE_STP_Det_126', 'probability': '0.08372*exp(-dist_2D/60.99)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 5, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L4_L6_TPC_L4':   {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_EE_STP_Det_126', 'probability': '0.07633*exp(-dist_2D/64.95)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 5, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L4_L6_BPC':      {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.11774*exp(-dist_2D/72.98)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L4_L6_IPC':      {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.09460*exp(-dist_2D/78.66)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_TPC_L4_L6_UTPC':     {'preConds': {'pop': L6_CT_byophisical_pops},                             'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_EE_STP_Det_116', 'probability': '0.12285*exp(-dist_2D/71.17)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EI_L6_BPC_L6_LBC':         {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6IN_LBC_bNA']},                                     'synMech': 'S1_EI_STP_Det_134', 'probability': '0.07003*exp(-dist_2D/49.67)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EI_L6_BPC_L6_LBC_B':       {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_cNA']},                     'synMech': 'S1_EI_STP_Det_115', 'probability': '0.07003*exp(-dist_2D/49.67)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EI_L6_BPC_L6_MC':          {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_EI_STP_Det_133', 'probability': '0.08406*exp(-dist_2D/54.24)*(dist_2D<375)', 'weight': 0.11, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 7, 'sec': 'spiny'},
                            'EE_L6_BPC_L6_TPC_L1':      {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_EE_STP_Det_116', 'probability': '0.11657*exp(-dist_2D/62.99)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_BPC_L6_TPC_L4':      {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_EE_STP_Det_116', 'probability': '0.11118*exp(-dist_2D/68.74)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_BPC_L6_BPC':         {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.12045*exp(-dist_2D/77.35)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_BPC_L6_IPC':         {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.09551*exp(-dist_2D/86.08)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_BPC_L6_UTPC':        {'preConds': {'pop': ['L6CC_BPC_cAD']},                                     'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_EE_STP_Det_116', 'probability': '0.12455*exp(-dist_2D/75.19)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EI_L6_IPC_L6_LBC':         {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_bNA', 'L6IN_LBC_cNA']},     'synMech': 'S1_EI_STP_Det_131', 'probability': '0.05854*exp(-dist_2D/48.88)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EI_L6_IPC_L6_MC':          {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_EI_STP_Det_133', 'probability': '0.07405*exp(-dist_2D/51.21)*(dist_2D<375)', 'weight': 0.11, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EE_L6_IPC_L6_TPC_L1':      {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_EE_STP_Det_129', 'probability': '0.10606*exp(-dist_2D/57.14)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_IPC_L6_TPC_L4':      {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_EE_STP_Det_129', 'probability': '0.08896*exp(-dist_2D/67.16)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_IPC_L6_BPC':         {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_128', 'probability': '0.10351*exp(-dist_2D/71.39)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_IPC_L6_IPC':         {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_127', 'probability': '0.08409*exp(-dist_2D/79.11)*(dist_2D<375)', 'weight': 0.8,  'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_IPC_L6_UTPC':        {'preConds': {'pop': ['L6CC_IPC_cAD']},                                     'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_EE_STP_Det_116', 'probability': '0.09635*exp(-dist_2D/74.30)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EI_L6_UTPC_L6_LBC':        {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6IN_LBC_bNA']},                                     'synMech': 'S1_EI_STP_Det_134', 'probability': '0.07835*exp(-dist_2D/54.00)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EI_L6_UTPC_L6_LBC_B':      {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6IN_LBC_bAC', 'L6IN_LBC_cNA']},                     'synMech': 'S1_EI_STP_Det_115', 'probability': '0.07835*exp(-dist_2D/54.00)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EI_L6_UTPC_L6_MC':         {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6IN_MC_bAC', 'L6IN_MC_bNA', 'L6IN_MC_cAC']},        'synMech': 'S1_EI_STP_Det_133', 'probability': '0.10244*exp(-dist_2D/56.42)*(dist_2D<375)', 'weight': 0.11, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 8, 'sec': 'spiny'},
                            'EE_L6_UTPC_L6_TPC_L1':     {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6CC_TPC_L1_cAD']},                                  'synMech': 'S1_EE_STP_Det_116', 'probability': '0.14799*exp(-dist_2D/59.55)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_UTPC_L6_TPC_L4':     {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': L6_CT_byophisical_pops},                             'synMech': 'S1_EE_STP_Det_116', 'probability': '0.14224*exp(-dist_2D/63.75)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_UTPC_L6_BPC':        {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6CC_BPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.15474*exp(-dist_2D/72.79)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                            'EE_L6_UTPC_L6_IPC':        {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6CC_IPC_cAD']},                                     'synMech': 'S1_EE_STP_Det_116', 'probability': '0.12570*exp(-dist_2D/81.42)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 3, 'sec': 'spinyEE'},
                            'EE_L6_UTPC_L6_UTPC':       {'preConds': {'pop': ['L6CC_UTPC_cAD']},                                    'postConds': {'pop': ['L6CC_UTPC_cAD']},                                    'synMech': 'S1_EE_STP_Det_116', 'probability': '0.16006*exp(-dist_2D/69.53)*(dist_2D<375)', 'weight': 0.72, 'delay': 'defaultDelay+dist_3D/propVelocity', 'synsPerConn': 4, 'sec': 'spinyEE'},
                        }
        
        # --- Adds the pop_flag to all 'pop' keys in 'preConds' and 'postConds'
        for conn_key, conn_data in TableS1Conns.items():
            if 'preConds' in conn_data and 'pop' in conn_data['preConds']:      conn_data['preConds']['pop'] = [pop_element + pop_flag for pop_element in conn_data['preConds']['pop']]
            if 'postConds' in conn_data and 'pop' in conn_data['postConds']:    conn_data['postConds']['pop'] = [pop_element + pop_flag for pop_element in conn_data['postConds']['pop']]
...
class LoadSimResults():
    def getSpikesFromConnectedCells(th_spk_dict, thal_gids, edge_sources_dict, showFig=False):
        # edge_sources_dict: must be a dict with all gids from pre cells grouped by pathway 
        ...
        for pop in th_spk_dict.keys():
            # print('Pop name: ', pop,'\t - # of cells: ',len(th_spk_dict[pop].keys()))
        ...
        for pop in th_spk_dict.keys():
            ...
            print('\nPop name: ', pop)
            print('\t all  cells: ', len(th_spk_dict[pop]))
            print('\t conn cells: ', len(th_connected_cells_dict[pop]))
            ...
            print('\t\t--- spiking cells: ', len(spiking_cells), '\tnon-spiking cells: ', len(non_spiking_cells))
    ...
    def getSpikesFromSim(filePath, cfg_file, microcircuit_number=None, showFig=False, exportJSON=False):
        f       = h5py.File(filePath, 'r')
        dset    = f['spikes']
        spkids  = list(dset['All']['node_ids'])
        spkts   = list(dset['All']['timestamps'])
        ...
        if exportJSON:
            print('Saving JSON template of spike times')
            with open('../conn/bbp_input_data/Fig4_wakefulness_spontaneous/spike_times.json', "w") as json_file: json.dump(th_spk_dict, json_file)
