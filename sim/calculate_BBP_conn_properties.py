...
for target_pop in target_pops.keys():
    ...
    if   'Rt'  in target_pop: mtype_flag='Rt'
    elif 'VPL' in target_pop: mtype_flag='VPL'
    target_mc_region='mc2;'+mtype_flag
    ...
    pathway_edges[target_pop] = NetPyNE_BBP.LoadBBPCircuit.getNodeEdges(    cfg.sonataConfigFile,
                                                                {'population': 'thalamus_neurons', 'mtype': target_pop, 'region':target_mc_region},
...
#     pathway_edges[target_pop] = NetPyNE_BBP.LoadBBPCircuit.getNodeEdges(    cfg.sonataConfigFile,
#                                                                 {'population': 'thalamus_neurons', 'mtype': target_pop, 'region':target_mc_region},
#                                                                 special_conditions={'inter_sources':target_pops[target_pop],'inter_targets':[],'select_microcircuit':2}
#                                                                 )

# # all projections -> all thalamus
# pathway_edges={}
# for target_pop in target_pops.keys():
#     pathway_edges.update({target_pop:{}})
#     print(' --- Calculating syns per conn for population ', target_pop)
...
if update_conn_properties:
    print('Updating conn_propeties file')
    ...
    print('Updated file in: ', conn_stats_fileName)