...
if cfg.connType == 'original':
    ...
    import numpy as np
    # https://www.neuron.yale.edu/neuron/static/py_doc/programming/math/vector.html#Vector.play 
    # https://www.neuron.yale.edu/phpBB/viewtopic.php?t=1802 
    # y = h.Vector(np.sin(t))

    # for cell in sim.net.cells:
    cell =sim.net.cells[0]
    if 'vecstim_pop' not in cell.tags['pop']:
        if cell.tags['cellType'] in sim.cfg.store_thresholdCurrent.keys():
            print('\t---> Modifying stims on cell #\t', cell.tags['cellType'])
            use_ornstein_uhlenbeck=True
            if use_ornstein_uhlenbeck:
                print('\t---> Using Ornstein Uhlenbeck noise')
                import math
                from CurrentStim import CurrentStim as CS

                mean_percect        = 0.0  # from BlueConfig file --- percent of threshold current (or holding current, not sure)
                threshold_current   = sim.cfg.store_thresholdCurrent[cell.tags['cellType']]
                holding_current     = sim.cfg.store_holdingCurrent[cell.tags['cellType']]
                
                mean                = (mean_percect/100)*threshold_current
                variance            = 0.001  # from BlueConfig file
                
                tvec,svec = CS.add_ornstein_uhlenbeck(tau=1e-9,sigma=math.sqrt(variance),mean=mean,duration=sim.cfg.duration,dt=0.25,seed=sim.cfg.base_random_seed,plotFig=True)
                svec.play(sim.net.cells[0].stims[0]['hObj']._ref_amp, tvec, True)
            
            else:
                t = h.Vector(np.linspace(0, sim.cfg.duration, int(sim.cfg.duration/sim.cfg.dt)))
                r = h.Random(sim.cfg.base_random_seed)

                mean_val = abs(sim.cfg.store_thresholdCurrent[cell.tags['cellType']]*1.6)
                # mean_val = abs(sim.cfg.store_thresholdCurrent[cell.tags['cellType']]*0)        # from BlueConfig file
                var_val  = abs(sim.cfg.store_thresholdCurrent[cell.tags['cellType']]*0.001)    # from BlueConfig file
                y = h.Vector([r.normal(mean_val,var_val) for t_ in np.linspace(0, sim.cfg.duration, int(sim.cfg.duration/sim.cfg.dt))])

                sim.net.cells[0].stims[0]['hObj'].delay=0
                sim.net.cells[0].stims[0]['hObj'].dur=1e9
                # y.play(sim.net.cells[0].stims[0]['hObj'].amp, t, True)
                y.play(sim.net.cells[0].stims[0]['hObj']._ref_amp, t, True)

                plotFigs=True
                if plotFigs:
                    plt.figure()
                    plt.scatter(list(t),list(y))
                    plt.savefig('test_fig_vec_'+cell.tags['cellType']+'.png')
                    
                    plt.figure()
                    plt.hist(list(y),100)
                    plt.savefig('test_fig_vec_'+cell.tags['cellType']+'_hist.png')
