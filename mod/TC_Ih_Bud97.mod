...
PARAMETER {
    ...
    celsius (degC)
	q10 = 4	     
	shift = 0 (mV) : Add shift parameter
}
...
PROCEDURE rates() {
	v = v + 0	
	mInf = 1 / (1 + exp((v + shift + 86.4) / 11.2)) : Budde et al., 1997, with shift
	mTau = (1 / (exp(-14.59 - 0.086 * v) + exp(-1.87 + 0.0701 * v))) / tcorr : From Huguenard, 1992
	v = v - 0
}
UNITSON
